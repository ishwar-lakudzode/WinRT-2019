...
{
	CMultiplicationDivision* pCMultiplicationDiv = NULL;
	HRESULT hr;
	if (pUnkOuter != NULL)
		return(CLASS_E_NOAGGREGATION);
	pCMultiplicationDiv = new CMultiplicationDivision;
	if (pCMultiplicationDiv == NULL)
		return(E_OUTOFMEMORY);
	hr = pCMultiplicationDiv->QueryInterface(riid, ppv);
	pCMultiplicationDiv->Release();
	return(hr);
}
...
{
	CMultiplicationDivisionClassFactory* pMultiplicationDivisionClassFactory = NULL;
	HRESULT hr;
	if (rclsid != CLSID_MultiplicationDivision)
		return (CLASS_E_CLASSNOTAVAILABLE);
	pMultiplicationDivisionClassFactory = new CMultiplicationDivisionClassFactory;
	if (pMultiplicationDivisionClassFactory == NULL)
		return(E_OUTOFMEMORY);
	hr = pMultiplicationDivisionClassFactory->QueryInterface(riid, ppv);
	pMultiplicationDivisionClassFactory->Release();
	return(hr);
}
